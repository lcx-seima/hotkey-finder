...
<div align="center">
  <img src="docs/assets/demo.gif" width="500" alt="Hotkey Finder detecting the app behind a keyboard shortcut">
</div>
...
[Download the latest Hotkey Finder](https://github.com/lcx-seima/hotkey-finder/releases/latest/download/Hotkey-Finder.zip), or visit [GitHub Releases](https://github.com/lcx-seima/hotkey-finder/releases/latest) to browse all versions.

> [!WARNING]
> The downloadable app is not Developer ID signed or notarized by Apple. Only install copies downloaded from this repository.
...
After launch, follow the in-app prompt to grant Input Monitoring access. Quit and reopen the app if macOS asks you to restart it.

<details>
<summary>Why does macOS show a security warning?</summary>

The download uses an ad-hoc signature rather than a paid Apple Developer ID certificate, so macOS cannot verify the developer or check the app through Apple's notarization service.

The **Open Anyway** action creates an exception for Hotkey Finder and does not require disabling Gatekeeper globally. The button is usually available for about an hour after a blocked launch attempt. Read [Apple's safety guidance](https://support.apple.com/en-us/102445) before overriding the warning. Installing an update may require granting privacy permissions again.
...
Download `Hotkey-Finder.zip.sha256` from the same release, then run:
